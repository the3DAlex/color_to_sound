...
namespace RandomAudioGenerator
{
    internal class wav
    {
        public static void WavMain()
        {
            var soundData = CreateSinWave(44100, 2000, 2.0f);
        ...
        public static byte[] CreateSinWave(int sampleRate, float frequency, float length)
        {
            int sampleCount = (int)(((double)sampleRate) * length);
            short[] tempBuffer = new short[sampleCount];
            byte[] retVal = new byte[sampleCount * 2];
            float step = (float)Math.PI * 2.0f / sampleRate * frequency;
            float current = 0;

            for (int i = 0; i < tempBuffer.Length; ++i)
            {
                tempBuffer[i] = (short)(Math.Sin(current) * ((double)short.MaxValue));
                current += step;
            }
